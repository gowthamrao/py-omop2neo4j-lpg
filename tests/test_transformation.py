...
class TestTransformation(unittest.TestCase):
    ...
    def test_prepare_for_bulk_import(self):
        ...
        self.assertListEqual(list(df_domain_nodes.columns), [':ID', 'domain_name', 'domain_concept_id', ':LABEL'])
        self.assertTrue(all(df_domain_nodes[':LABEL'] == 'Domain'))
        self.assertEqual(df_domain_nodes[df_domain_nodes[':ID'] == 'Drug']['domain_name'].iloc[0], 'Drug')
        ...
        self.assertEqual(df_vocab_nodes[df_vocab_nodes[':ID'] == 'RxNorm']['vocabulary_name'].iloc[0], 'RxNorm')

        # Concept Nodes
        df_concept_nodes = pd.read_csv(os.path.join(self.test_import_dir, 'nodes_concept.csv'), dtype=str)
        ...
        df_domain_rels = pd.read_csv(os.path.join(self.test_import_dir, 'rels_in_domain.csv'), dtype=str)
        self.assertEqual(len(df_domain_rels), 3)
        self.assertListEqual(list(df_domain_rels.columns), [':START_ID', ':END_ID', ':TYPE'])
        self.assertTrue(all(df_domain_rels[':TYPE'] == 'IN_DOMAIN'))
        self.assertEqual(df_domain_rels[df_domain_rels[':START_ID'] == '1001'][':END_ID'].iloc[0], 'Drug')

        # FROM_VOCABULARY Relationships
        df_vocab_rels = pd.read_csv(os.path.join(self.test_import_dir, 'rels_from_vocabulary.csv'), dtype=str)
        self.assertEqual(len(df_vocab_rels), 3)
        self.assertListEqual(list(df_vocab_rels.columns), [':START_ID', ':END_ID', ':TYPE'])
        self.assertTrue(all(df_vocab_rels[':TYPE'] == 'FROM_VOCABULARY'))
        self.assertEqual(df_vocab_rels[df_vocab_rels[':START_ID'] == '1001'][':END_ID'].iloc[0], 'RxNorm')

        # Semantic Relationships
        df_semantic_rels = pd.read_csv(os.path.join(self.test_import_dir, 'rels_semantic.csv'), dtype=str)
        ...
        df_ancestor_rels = pd.read_csv(os.path.join(self.test_import_dir, 'rels_ancestor.csv'), dtype=str)
        self.assertEqual(len(df_ancestor_rels), 1)
        self.assertListEqual(list(df_ancestor_rels.columns), [':START_ID', ':END_ID', 'min_levels:int', 'max_levels:int', ':TYPE'])
